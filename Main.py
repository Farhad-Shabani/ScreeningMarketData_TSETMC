...
def Export_Database(portfolio,Selected_Stocks):
    ...
    for i in Selected_Stocks_Map:
        ws['{}18'.format(Selected_Stocks_Map[i])] = Selected_Stocks_Database[i][0]  
        ws['{}19'.format(Selected_Stocks_Map[i])] = Selected_Stocks_Database[i][11]
        ws['{}20'.format(Selected_Stocks_Map[i])] = Selected_Stocks_Database[i][12]


    # ValueSort -----------------------------------------------------------------------------------------------------
    for j in range(10):
        ws['M{}'.format(25+j)] = ValueSort[j][0]
        ws['N{}'.format(25+j)] = ValueSort[j][8]
